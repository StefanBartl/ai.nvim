describe("ai.bindings.edit", function()
  ...
  describe("resolve_range", function()
    ...
    it("swaps a backwards explicit range instead of passing it through", function()
      local line1, line2 = edit.resolve_range({ line1 = 4, line2 = 2 }, winid)
      assert.are.equal(2, line1)
      assert.are.equal(4, line2)
    end)
  end)

  describe("buffer_changed", function()
    it("is false for a valid buffer whose changedtick still matches", function()
      local bufnr = vim.api.nvim_create_buf(false, true)
      local tick = vim.api.nvim_buf_get_changedtick(bufnr)
      assert.is_false(edit.buffer_changed(bufnr, tick))
      vim.api.nvim_buf_delete(bufnr, { force = true })
    end)

    it("is true once the buffer's changedtick has moved on", function()
      local bufnr = vim.api.nvim_create_buf(false, true)
      local tick = vim.api.nvim_buf_get_changedtick(bufnr)
      vim.api.nvim_buf_set_lines(bufnr, 0, -1, false, { "edited" })
      assert.is_true(edit.buffer_changed(bufnr, tick))
      vim.api.nvim_buf_delete(bufnr, { force = true })
    end)

    it("is true for a deleted buffer, regardless of the tick given", function()
      local bufnr = vim.api.nvim_create_buf(false, true)
      local tick = vim.api.nvim_buf_get_changedtick(bufnr)
      vim.api.nvim_buf_delete(bufnr, { force = true })
      assert.is_true(edit.buffer_changed(bufnr, tick))
